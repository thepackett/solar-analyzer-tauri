    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        let paste_visible = match &self.copy_state {
            Some((data, _update_callback)) => {
                if let Some(SharableGraphData::SecYAxisData(_)) = data.as_ref() {
                    true
                } else {
                    false
                }
        ...
        match msg {
            ...
            Self::Message::Copy => {
                if let Some((_data, update_shared_data)) = &self.copy_state {
                    update_shared_data.emit(Rc::from(Some(SharableGraphData::SecYAxisData(props.current_state.clone()))));
                }
                return true;
            },
            Self::Message::Paste => {
                if let Some((data, _update_shared_data)) = &self.copy_state {
                    if let Some(SharableGraphData::SecYAxisData(saved_sec_y_axis_request)) = data.as_ref() {
                        props.callback.emit(saved_sec_y_axis_request.clone());
                    }
                }
            },
        }
